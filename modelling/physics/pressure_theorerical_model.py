...
def pressure_dry(h):
    integral, err = quad(lambda h: 1 / ISA_temperature(h), 0, h, limit=100, points=[0, H1, 20000, 32000, 47000, 51000, 71000, 84852])
...
def calc_chi(RH):
    integral_frac, _ = quad(lambda z: water_molar_fraction(RH,h=z) * pressure_dry(z) / ISA_temperature(z), 0, 84852, limit=100, points=[0, H1, 20000, 32000, 47000, 51000, 71000, 84852])
    integral_norm, _ = quad(lambda z: pressure_dry(z) / ISA_temperature(z), 0, 84852, limit=100, points=[0, H1, 20000, 32000, 47000, 51000, 71000, 84852])
...
def pressure_moist(h,RH,chi):
    integral, _ = quad(lambda z: water_molar_fraction(RH,h=z) / ISA_temperature(z), 0, h, limit=100, points=[0, H1, 20000, 32000, 47000, 51000, 71000, 84852])
...
#altitudes = np.linspace(0, 800, 500)
...
#temperatures_smaller_mod = [ISA_temperature_up_to_1000(alt,T_surf=298.15,L_0=5.0) for alt in altitudes]
